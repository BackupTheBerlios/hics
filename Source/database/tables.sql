...
CREATE TABLE Kunde ( KundenNr SERIAL PRIMARY KEY, Nachname VARCHAR(50) NOT NULL, Vorname VARCHAR(50) NOT NULL, Land VARCHAR(5) DEFAULT 'A', PLZ INTEGER, Wohnort VARCHAR(50), Strasse VARCHAR(50), TelNr VARCHAR(25) );
CREATE TABLE Notiz ( NotizNr SERIAL UNIQUE NOT NULL, KundenNr INTEGER NOT NULL, Bezeichnung VARCHAR(1000) NOT NULL, PRIMARY KEY (NotizNr, KundenNr), FOREIGN KEY (KundenNr) REFERENCES Kunde(KundenNr) );
